<!-- `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  <?php
   ...
   class observacionBoletin extends ConectarPDO{
		...
		public function agregar(){
			$this->sql = "INSERT INTO observaciones_area(periodo, idMatricula, curso, asistencia, cumplimiento, observacion, anho) VALUES(?,?,?,?,?,?,?)";
			try {
				$stm = $this->Conexion->prepare($this->sql);
				$stm->bindParam(1, $this->periodo);
				$stm->bindParam(2, $this->idMatricula);
		...
		public function eliminar($tabla){
			$this->sql = "DELETE FROM areasxsedes_2 WHERE id='".$this->id."'";
			try {
				$stm = $this->Conexion->prepare($this->sql);
				$stm->execute();
			} catch (Exception $e) {
		...
		public function listar(){
			$this->sql = "SELECT * FROM areasxsedes_2 WHERE codSede ='".$this->codSede."' AND anho='".$this->anho."' ORDER BY Nombre, Abreviatura ASC";
			try {
   			$stm = $this->Conexion->prepare($this->sql);
   			$stm->execute();
   			$reg = $stm->fetchAll(PDO::FETCH_ASSOC);  

...
<html>
    <head>
        ...
        <link href="../complementos/AdminLTE/bootstrap/css/bootstrap.css" rel="stylesheet">
        <style>
            .ocultar{
                width:100%;
                height:50px;
    ...
    <body>
        <?php if (!isset($_POST['boletinEstudiante'])) { ?>
            <div class='ocultar'>
                <form action='http://localhost/marsoft/Controladores/ctrlBoletin.php' method='post'  style='float:left;'>
        ...
        if($periodoBol != 'Final'){//Verifico cual periodo se quiere consultar si es diferente del final se imprime el periodo    correspondiente
            ...
            ?>            
                <div >
                    <?php include("encabezadoM1.php") ?>
                    <?php 
                        foreach ($sqlAreas as $area) {
                            include("../vistas/boletines/bloqueAreas-M1.php");

                            $objNumAsig = new Area();
                            $objNumAsig->idArea = $area['id'];
                            $objNumAsig->idGrado = $area['idGrado'];
                            if ($objNumAsig->conteoAsignaturas() >0) {
                                $objAsig = new Asignatura();
                                $objAsig->idArea = $area['id'];
                                foreach ($objAsig->listar() as $asignatura) {
                                    include("../vistas/boletines/bloqueAsignaturas-M1.php");
                                }
                            }
                        ?>  
               <!-- 
